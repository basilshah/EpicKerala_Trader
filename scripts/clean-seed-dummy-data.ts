...
const seedPath = join(process.cwd(), 'prisma', 'seed.ts');let content = readFileSync(seedPath, 'utf-8');

// Remove certificationFiles JSON.stringify blocks
content = content.replace(
  /\s*certificationFiles:\s*JSON\.stringify\(\[[\s\S]*?\]\),?\n/g,
  '\n'
);

// Remove catalogs JSON.stringify blocks  
content = content.replace(
  /\s*catalogs:\s*JSON\.stringify\(\[[\s\S]*?\]\),?\n/g,
  '\n'
);
